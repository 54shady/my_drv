...
static int lcd_init(void)
{
	...
	s3c_lcd->fix.smem_len = 320*240*16/8;        /* MINI2440的LCD位宽是24,但是2440里会分配4字节即32位(浪费1字节) */
...
*/
	*gpccon &= ~(0xffc003cc);
    *gpccon  |= 0xaa955699;   /* GPIO管脚用于VD[7:0],LCDVF[2:0],VM,VFRAME,VLINE,VCLK,LEND */

	*gpdcon &= ~(0xffc0fff0);
	*gpdcon  |= 0xaa95aaa1;   /* GPIO管脚用于VD[23:8] */
	
//	*gpbcon &= ~(3);  /* GPB0设置为输出引脚 */
//	*gpbcon |= 1;
//	*gpbdat &= ~1;     /* 输出低电平 */

	*gpgcon |= (3<<(4*2)); /* GPG4用作LCD_PWREN */
	 ...
	 *            CLKVAL = 6
	 *            5.48MHz = 100MHz / [(CLKVAL+1) x 2]
	 *            CLKVAL = 8
	 * bit[6:5]: 0b11, TFT LCD
	 * bit[4:1]: 0b1101, 24 bpp for TFT
	 * bit[0]  : 0 = Disable the video output and the LCD control signal.
	 */
	//lcd_regs->lcdcon1  = (6<<8) | (3<<5) | (0x0d<<1);//CLKVAL = 6
//	lcd_regs->lcdcon1  = (7<<8) | (3<<5) | (0x0d<<1);//CLKVAL = 8
	lcd_regs->lcdcon1 = 0x00000878;
...
//	lcd_regs->lcdcon2  = (0<<24) | (320<<14) | (4<<6) | (9<<0);
	lcd_regs->lcdcon2  = 0x004fc109;
	//10011111100000100001001
...
//	lcd_regs->lcdcon3 = (0<<19) | (240<<8) | (25<<0);
	lcd_regs->lcdcon3 = 0x00c8ef00;
	/* 水平方向的同步信号
	 * bit[7:0]	: HSPW, HSYNC信号的脉冲宽度, LCD手册Thp=1, 所以HSPW=1-1=0
	 */	
//	lcd_regs->lcdcon4 = (4<<0);
	lcd_regs->lcdcon4 = 0x00000004;
	/* 信号的极性 
	 ...
	 * bit[0] : 0 = HWSWP
	 */
	//lcd_regs->lcdcon5 &= ~((1<<12) | (1<<1) | (1<<0));
	lcd_regs->lcdcon5 = 0x00000f41;
...
//	lcd_regs->lcdsaddr1  = (s3c_lcd->fix.smem_start >> 1) & ~(3<<30);
	...
	lcd_regs->lcdsaddr1  = 0x19c60000;
	lcd_regs->lcdsaddr2  = 0x19c72c00;
	lcd_regs->lcdsaddr3  = 0x000000f0;
//	lcd_regs->lcdsaddr2  = s3c_lcd->fix.smem_start;
	//lcd_regs->lcdsaddr2 += s3c_lcd->fix.line_length * s3c_lcd->var.yres;
	//lcd_regs->lcdsaddr2 >>= 1;

//	lcd_regs->lcdsaddr3  = (320*32/16);  /* 一行的长度(单位: 2字节) */	
	//lcd_regs->lcdsaddr3 = (0<<11) |((s3c_lcd->fix.line_length / 2) & 0x3ff);
	//  3*240 / 2 & 0x3ff = 
